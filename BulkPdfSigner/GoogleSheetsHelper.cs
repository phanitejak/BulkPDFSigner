...
namespace BulkPdfSigner
{
    public class GoogleSheetsHelper
    {
        ...
        public GoogleSheetsHelper(string credentialFileName, string spreadsheetId, string credEmail, bool isServiceCredential = false)
        {
            GoogleCredential credential;

            if (isServiceCredential)
            {
                var certificate = new X509Certificate2(credentialFileName, "notasecret", X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.Exportable);
                var creds = new ServiceAccountCredential(new ServiceAccountCredential.Initializer(credEmail)
                {
                    Scopes = scopes
                }.FromCertificate(certificate));

                credential = GoogleCredential.FromServiceAccountCredential(creds);
            }
            else
            {
                credential = GoogleCredential.FromStream(new FileStream(credentialFileName, FileMode.Open)).CreateScoped(scopes);
            }
        ...
        public DataTable GetDataTableFromSheet(GoogleSheetParameters googleSheetParameters)
        {
            DataTable dt = new DataTable("LicenseDetails");
